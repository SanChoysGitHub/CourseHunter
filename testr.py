...
def print_weather(weather, lang):
...
def get_weather(event='', lang=''):
    if not entry.get():
        ...
        label_low['text'] = print_weather(weather, lang)
...
settings_menu_lang.add_command(label='EN', command=lambda: get_weather(lang='en'))
settings_menu_lang.add_command(label='UA', command=lambda: get_weather(lang='ua'))
settings_menu_lang.add_command(label='RU', command=lambda: get_weather(lang='ru'))
